...
export function DataTable<TData, TValue>({ columns, data, pagination = true }: DataTableProps<TData, TValue>) {
  ...
  return (
    <>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow className="border-b border-gray-200" key={row.id} data-state={row.getIsSelected() && "selected"}>
      ...
      {/* {pagination && <Paginator currentPage={table.getState().pagination.pageIndex + 1} totalPages={100} onPageChange={(pageNumber) => table.setPageIndex(pageNumber - 1)} showPreviousNext />} */}
    </>
  );
}
